  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const reletedData = await ApiServices.fetching(
          `search?part=snippet&reletedToVideoId${id}&type=video`
        );
    ...
    <Box minHeight={"90vh"} mb={10}>
      <Box display={"flex"} sx={{ flexDirection: { xs: "column", md: "row" } }}>
        <Box width={{ xs: "100%", md: "75%" }}>
          ...
          {videoDetail?.snippet?.tags?.map((item, idx) => (
            <Chip
              label={item}
              key={idx}
              sx={{ marginTop1: "10px", cursor: "pointer", ml: "10px" }}
              deleteIcon={<Tag />}
              onDelete={() => {}}
              variant="outlined"
            />
            ...
            <Link to={`/channel/${videoDetail?.snippet?.channelId}`}>
              ...
              >
                <Avatar
                  ...
                  url={videoDetail.snippet.thumbnails.default.url}
                />
        ...
        <Box
          ...
          maxHeight={"120vh"}
        >
          <Videos video={reletedVideo && reletedVideo} />
        </Box>
      </Box>
    </Box>
  );
